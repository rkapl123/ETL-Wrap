use strict;
use Data::Dumper; use Log::Log4perl qw(get_logger);
use ETL::Wrap; use ETL::Wrap::Common; use ETL::Wrap::DateUtil;
...
foreach my $job (keys %{$config{checkLookup}}) {
	...
	ETL::Wrap::Common::sendGeneralMail("", $mailsendTo,"","",($execute{envraw} ? $execute{envraw}.": " : "").'job starting problem','TEXT',$infos);
}
__END__
=head1 NAME
...
checkLogExist.pl -l YYYYMMDD

checkLogExist is called frequently (LogWatchExecution Job) and checks if defined log entries exist in the defined log files, (hinting that the job was run/started), resp. whether the Logfile exists at all.

Configuration is done in sub-hash $config{checkLookup} (same place for the errmailaddress/errmailsubject of the error mails for the jobs themselves):

$config{checkLookup} = {
<nameOfJobscript(without.pl)> => {errmailaddress => "test\@test.com", errmailsubject => "testjob failed", timeToCheck => "0800", freqToCheck => "B", logFileToCheck => "test.log", logcheck => "started.*"},

errmailaddress ..  where should the mail sent to in case of non-existence or an error.
errmailsubject ..  subject-line for error mail, only used for error mail sending in the taks itself.
timeToCheck    ..  all checks earlier than this are ignored
freqToCheck    ..  ignore log check except on: ML..Monthende, D..every day, B..Business days, M1..Month-beginning, W{n}..Weekday (n:1=Sunday-7=Saturday) 
logFileToCheck ..  Where (which logfile) should the job have written into ?
logcheck       ..  "regex keyword/expression" to compare the rows, if this is missing in the logfile after the current date/timeToCheck then an alarm is sent to the configured errmailaddress
